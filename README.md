...
Последний опубликованный релиз — `v0.7.0`. Версия `0.8.0` находится на стадии Stabilization и публично не публиковалась.
...
- Версия `0.8.0` — Stabilization: техническая сборка не означает Stable; для регулярного использования доступна `v0.7.0`.
...
- Стадия: `Stabilization`.
- Последняя проверенная стабильная версия: `v0.7.0`.
- Текущий TPM 0.8.0-debug частично проверен в Robur Genplan 16.0.62.12 (установка, About, применение alias без перезапуска); полный функциональный smoke выполнен для dev.9.
