...
- Robur: поддерживается Robur 16.0. Проверено: Robur Genplan 16.0 (сборка 16.0.62.12) — полный smoke для `0.8.0-dev.9` и все ручные сценарии для `0.8.0-rc.1` (2026-09-24; код идентичен `v0.8.0`, отличается только версия в метаданных). Ранние версии дополнительно проверялись в Robur Road 16.0 и учебной версии; `v0.7.0` проверялась в Robur Genplan 16.0.
...
Последний опубликованный релиз — `v0.7.0`. Версия `v0.8.0` завершена как Stable (2026-09-24) и будет опубликована отдельным шагом.
...
- `О плагине` показывает актуальную версию и стадию (например, `0.8.0`), путь фактически загруженной DLL и состояние лога — сверьте DLL path с установленной версией.
...
- Стабильная версия — `v0.8.0` (Stable применён 2026-09-24); последний опубликованный релиз на момент синхронизации — `v0.7.0`.
...
- Проверенный артефакт: `0.8.0-rc.1.tpm` (SHA-256 `ff3b7390…9784`) — все ручные сценарии без ошибок в Robur Genplan 16.0.62.12; сборка `v0.8.0` отличается только версией в метаданных; после сборки Stable-TPM — контрольная установка.
