...
## Cursor
Last completed action: подтверждённый lifecycle APPLY RC → Stable (2026-09-24; разрешённый CHECK того же дня, явное подтверждение пользователя): версия 0.8.0, стадия Stable, метаданные csproj/About, Release-сборка 0 warning / 0 error, Ui.Tests 49 PASS; release artifact не создавался
Next action: отдельно собрать Stable-артефакт dist/RoburPseudoCommands-0.8.0.tpm (scripts/build-tpm.ps1, Release) и выполнить контрольную установку в host (About «Stable / 0.8.0» + базовый сценарий); release note и публикация — отдельными сценариями по подтверждению
