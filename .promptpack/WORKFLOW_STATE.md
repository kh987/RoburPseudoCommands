...
Draft revision: n/a
Draft status: n/a
Current step: Debug-стадия; документация синхронизирована (карточка + README); готовность к подготовке CHECK Debug → Stabilization
...
## Cursor
Last completed action: README.md обновлён (0.8.0-debug: popup-only aliases без ввода в командной строке, коэффициент маски, совместимость/проверки/ограничения, настройки AppData, сборка build-tpm.ps1; формат исходный — без BOM, LF; commit df5ee58)
Next action: пользователь запускает CHECK Debug → Stabilization из 73_LIFECYCLE_GATES.md (документация синхронизирована; из предстоящих пунктов подготовки — validator на машине с Python перед release route)
