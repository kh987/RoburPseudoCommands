...
| R7 | Сборка, упаковка и host smoke успешны; пользовательские конфиги принимаются | S5: build 0/0; TPM верифицирован; S6: smoke пройден | build 0/0 + smoke-матрица dev.9; адресные проверки debug TPM | dev.9.tpm (SHA-256 2AABFC76…4EDF35E): полный smoke 2026-09-20, лог 0 exception/error, aliases.json/settings.json приняты. Установленный 0.8.0-debug.tpm (SHA-256 A632C5E4…E2FFA): пользователь 2026-09-23 подтвердил установку, About, restart-free apply и три адресных отказных сценария без проблем; полный функциональный smoke этого TPM отдельно не заявлен | Verified |

Применимость полного smoke dev.9 к точному 0.8.0-debug.tpm для RC оценивается отдельно в новом CHECK; запись Verified у R7 основана на dev.9 и не означает отдельного полного smoke debug TPM.
